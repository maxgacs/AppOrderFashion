...
class MainActivityCus : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main_cus)

        val transaction = supportFragmentManager.beginTransaction()
        transaction.replace(R.id.nav_host_fragment, HomeCusFragment())
        transaction.commit()


        val callback = onBackPressedDispatcher.addCallback(this) {

            val builder: AlertDialog.Builder = AlertDialog.Builder(this@MainActivityCus)
        ...
        navView.setOnNavigationItemSelectedListener {
            var fm: Fragment = HomeCusFragment()
            when (it.itemId) {
                R.id.nav_home -> fm = HomeCusFragment()
                R.id.nav_cart -> fm = CartFragment()
                R.id.nav_Order -> fm = OrderFragment()
                R.id.nav_profile -> fm = ProfileCusFragment()
            }
